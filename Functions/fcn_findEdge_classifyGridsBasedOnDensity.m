...
%
%      total_N_points_in_each_grid: total number of points in each grid
%
%      point_density: Density of the points
%
%      gridCenters
%
% (OPTIONAL INPUTS):
...
%   See the script:
%   
%   script_test_fcn_findEdge_classifyGridsBasedOnDensity
%
...
format_1=sprintf(' ''.'',''Color'',[0.8, 0.8, 0.8],''MarkerSize'', 25,''DisplayName'', "Grids with low point density" ');
if (5<=nargin)
    temp = varargin{1};
    if ~isempty(temp)
    format_1=temp;
...
format_2=sprintf(' ''.'',''Color'',[0.2, 0.2, 0.2],''MarkerSize'', 25,''DisplayName'', "Grids with required density" ');
if (6<=nargin)
    temp = varargin{2};
    if ~isempty(temp)
    format_2=temp;
...
if flag_do_plots
    ...
    %low point density
    fcn_findEdge_plotLIDARLLA(plot_gridCenters_low_point_density,[],[],[],[],[],format_1,fig_num)
    hold on
